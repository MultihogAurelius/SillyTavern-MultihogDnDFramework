...
Map Architect receives the exact site root, entrance label, kind (`DUNGEON`, `INTERIOR`, or `SETTLEMENT`), scale, threat (`NONE` / `LOW` / `MODERATE` / `HIGH` / `DEADLY`), premise, optional `attachTo`, optional settlement-only `include[]`, and a configurable recent-story lookback. `attachTo.site` names an exact existing parent map and `attachTo.cell` names an exact AREA on it, so nested maps can be created from anywhere without player movement or a placeholder BUILDING. Explicit offsite attachment starts the new entrance UNREVEALED and never alters the footer; active-location shorthand retains the normal VISITED entrance. Defaults are DUNGEON=HIGH, SETTLEMENT=MODERATE, and INTERIOR=LOW. Scale is geographic size; threat is site danger and is never matched to party level. It emits one version 3 JSON object internally. Before persistence, a strict validator checks identity, kind, scale-appropriate area count, stable IDs, kind restrictions, enum values, references, reciprocal passages, and reachability.
...
The graphical **Map Editor** can create a Location root and version-3 map without an AI pass, attach a hand-built map to an existing unmapped root, or edit any stored AI/manual map. Its auto-layout graph, structured forms, and raw JSON view all mutate one undoable in-memory draft. Clicking an area node opens its edit/add/delete menu; dragging a curved connection from either side socket onto another node creates the reciprocal route directly on the graph. Strict validation and optimistic concurrency checks run before the atomic lorebook save. Hosted DUNGEON/INTERIOR creation reuses the same gateway promotion, breadcrumb derivation, host stamping, and nesting limit as Map Architect. Portable `multihog_map_package_v1` JSON carries map topology plus suggested CORE/keywords while removing campaign host binding and causal provenance.
