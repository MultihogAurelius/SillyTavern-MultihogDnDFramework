...
**Status:** Structured current-state and dedicated Map Architect are in play.
...
- Map Architect first creates and validates topology, then populates that locked graph in a separate content pass before writing the complete map to the root Location entry. `site` is the name of the map being created and may be invented before it appears in the Location footer; after creation the narrator copies that exact name into the footer. Human-readable map labels follow the campaign language; JSON keys, IDs, and enums stay English. Map activation still keys off an exact footer match to the saved site.
...
Map Updater is a dedicated one-shot JSON pass (own Map Updater connection, separate from Map Architect and Map Evolution). It emits either `{"noop":true}` or an occupancy transaction. Lorebook Agent no longer receives `inspect_map`, `list_map_assets`, `commit.map`, or `[MAP_COMMIT]`.
