...
describe('paired history retention', () => {
    it('keeps the newest 50 paired stones while preserving live state', () => {
        const s = history();
        trimMemoAndMapHistory(s);
        expect(s.memoHistory).toEqual(history(50).memoHistory);
        expect(s.dungeonMapHistory).toEqual(history(50).dungeonMapHistory);
    ...
    it.each([1, 49, 50, 233])('preserves LIVE and its map when LIVE starts at %s', live => {
        const s = history(234, live);
        const liveMap = s.dungeonMapHistory[live];
        trimMemoAndMapHistory(s);
        expect(s.memoHistory).toHaveLength(50);
        expect(s.dungeonMapHistory).toHaveLength(50);
        expect(s.historyIndex).toBe(Math.min(live, 49));
        expect(s.memoHistory[s.historyIndex]).toBe(s.currentMemo);
        expect(s.dungeonMapHistory[s.historyIndex]).toBe(liveMap);
        expect(s.memoHistory.slice(0, 49)).toEqual(history(49).memoHistory);
    });
    ...
    it('pads missing map snapshots without guessing occupancy', () => {
        ...
        expect(s.dungeonMapHistory.slice(1)).toEqual(Array(49).fill(null));
    });

    it('keeps 50 → 50 through repeated ordinary State Tracker commits', () => {
        const s = history(50);
        for (let i = 0; i < 120; i++) {
        ...
        expect(s.memoHistory[49]).toBe('turn 70');
        expect(s.dungeonMapHistory[49]).toEqual({ maps: ['turn map 70'] });
    });

    it('cleans existing live, inactive-chat, and profile histories on first load', () => {
        testExtensionSettings[MODULE_NAME] = {
            ...history(234, 200),
            chatStates: { inactive: history(234, 100), small: history(2) },
            profiles: { saved: history(234, 233) },
        };
        const s = getSettings();
        for (const snapshot of [s, s.chatStates.inactive, s.profiles.saved]) {
            expect(snapshot.memoHistory).toHaveLength(50);
            expect(snapshot.dungeonMapHistory).toHaveLength(50);
        ...
        expect(s.memoHistoryRetentionVersion).toBe(1);
    });
    ...
    it('repairs legacy object stones before applying retention', () => {
        ...
        expect(loaded.memoHistory[49]).toBe('memo 59');
        expect(loaded.dungeonMapHistory[49]).toEqual({ maps: ['map 59'] });
        expect(loaded.historyIndex).toBe(49);
    });
});
