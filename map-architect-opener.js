...
- Use these exact field names: site, entrance, kind (DUNGEON, SETTLEMENT, or INTERIOR), scale (SMALL, MEDIUM, or LARGE), threat (NONE, LOW, MODERATE, HIGH, or DEADLY), prompt, brief_description, optional attach_to_site plus attach_to_cell, and optional include. prompt may contain detailed private generation guidance. brief_description is stored as the Location CORE General field and as parent-gateway detail; never copy the full prompt into it. include must be a JSON array of exact existing DUNGEON/INTERIOR names and is allowed only while first creating a SETTLEMENT.
