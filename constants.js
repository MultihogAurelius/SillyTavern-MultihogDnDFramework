...
- Within SETTLEMENT, ordinary named shops, inns, chapels, and houses are BUILDING assets with no peer map; props such as stalls, wells, statues, altars, and chests are OBJECT.
- A map-worthy high-risk settlement site is SUBDUNGEON → peer DUNGEON. A significant lower-risk multi-room site is SUBINTERIOR → peer INTERIOR. Calling CreateAreaMap for the exact BUILDING/SUB* name is the explicit promotion signal. Never promote ordinary structures speculatively.
- If a settlement is implied, map or reuse the SETTLEMENT first and host any map-worthy nested peer inside it. Isolated DUNGEON/INTERIOR sites remain standalone.
- Wilderness, roads, alleys, streets, districts, BUILDING, and OBJECT never receive their own maps.
...
Tool results and DUNGEON_REALITY are private objective canon. Reveal only what the player perceives. DUNGEON and INTERIOR use room-scale layout; SETTLEMENT uses districts. Footer paths remain coarse-to-fine. BUILDING keeps the settlement active; a mapped peer becomes active at the deepest matching footer segment. Hosted peer reality includes a compact host brief for returning to the settlement.
</dungeon_reality_and_hidden_mapping>`,
