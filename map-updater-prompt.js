...
KIND: SETTLEMENT
- A chapel, inn, shop, house, or similar ordinary structure the party actually enters is a BUILDING asset in the current district, not a new area or child map. 
- OBJECT is props only. 
- BUILDING is a lightweight container with no rooms or connections. Contained occupants and things use the BUILDING id as location. Named people are CREATURE; unnamed bands, watches, and crowds are GROUP with count. Entering it is durable even when the district was already VISITED. 
- If CURRENT LOCATION names an untracked ordinary structure (e.g. "Chapel of the Drowned Stone", "Hollow Creek General Store"), ADD_ASSET kind BUILDING, knowledge KNOWN. Positional footer tails such as "behind the general store", "outside the inn", "near the chapel", or "in front of the gas station" place the party in the district relative to a landmark — they are NOT a new BUILDING and do not mean the party entered that structure. 
- Prefer the existing landmark asset when narration establishes a true first entry inside it; exterior-relative phrasing alone must never invent an asset named after the prepositional phrase. 
- Streetscape observation is also durable: when RECENT STORY clearly identifies existing UNREVEALED or SUSPECTED BUILDING/OBJECT landmarks from outside (a general store, gas station, diner, chapel facade, and so on), SET_ASSET knowledge KNOWN on each matching asset id. Match by the asset's name even when the prose uses a shorter common label ("gas station" → Bullion Gas Station). 
- Observation alone is not entry — do not clear notEntered or invent interiors unless a PRE-NARRATION BUILDING INTENT POPULATION bundle is supplied or the party actually goes inside.
- SUBDUNGEON/SUBINTERIOR are allowed only when the narration explicitly establishes a map-worthy child; never infer or perform BUILDING promotion. CreateAreaMap is the sole promotion signal.
- An explicit GM-authored rumor that a specific BUILDING contains a person, group, object, loot, hazard, or trap may ADD_ASSET at that BUILDING with knowledge SUSPECTED; certain knowledge may add the corresponding asset as KNOWN. Player speculation is not evidence. Rumor seeding never changes the BUILDING's notEntered flag; first entry still reconciles its suspected contents.
- The narrator may make mistakes in the footer, for example writing (Location: Bullion, Main Street, General Store) when the correct footer would be (Location: Bullion, Main Street, Bullion General Store) per a BUILDING Asset "Bullion General Store." However, use common sense and still treat them as being inside Bullion General Store despite the Footer only saying Main Street, General Store. Set the notEntered flag to false accordingly and consider Asset creation normally inside the BUILDING.

BUILDING entry and Asset population:
- When PRE-NARRATION BUILDING INTENT POPULATION is supplied, the player has deliberately targeted that BUILDING for approach, entry, or interaction. Establish its objective contents before the narrator adjudicates the action. Do not assume entry or a roll succeeded. Every newly generated child is UNREVEALED and no chronicle is allowed; later narration and the normal post-turn pass reveal only what the player actually perceives.
- When FIRST-ENTRY BUILDING POPULATION is supplied, populate or intentionally resolve that exact building and explicitly SET_ASSET notEntered:false in the same transaction; never noop that bundle.
- New interior contents use ADD_ASSET with name, kind, and location set to the BUILDING id/name. SET_ASSET is only for assets that already exist on the map (including clearing notEntered on the BUILDING itself). Never invent an asset_id and SET_ASSET it.
- Populate only map-worthy contents: CREATURE, GROUP, LOOT, HAZARD, TRAP, ALARM, BARRIER, and OBJECT/EFFECT when they are interactive, scavengable, dangerous, or plot-relevant. Do not ADD_ASSET ambient scenery or set dressing — tipped chairs, dusty booths, ordinary counters, broken glass, general mess, or flavor furniture belong in the BUILDING detail or narration, not as child assets.
- Derive how many and what kind of assets to add from the narrative and setting. If the area is infested or occupied, consider a fitting hostile; if already crowded with hostiles, do not pack every house with more. Prefer a few meaningful LOOT/OBJECT/HAZARD entries over many trivial props. A wealthy site may hold better scavengables than a stripped one. An established empty, closed, or thoroughly looted building may add no contents. Always adapt to the setting while keeping the map lean and interesting.
...
Streetscape observation (raise landmark knowledge without entry):
{"operation_id":"day2-0016-main-street-seen","operations":[{"op":"SET_ASSET","evidence":"CONFIRMED","asset_id":"bullion-gas-station","knowledge":"KNOWN","cause":"The party saw the gas station from Main Street."},{"op":"SET_ASSET","evidence":"CONFIRMED","asset_id":"bullion-diner","knowledge":"KNOWN","cause":"The party saw the diner from Main Street."},{"op":"SET_ASSET","evidence":"CONFIRMED","asset_id":"main-street-undead-pack","knowledge":"KNOWN","detail":"Five undead drift between the Main Street storefronts.","cause":"The party observed the pack on Main Street."}]}
...
Before answering, silently verify: valid JSON; exact existing IDs unless ADD_*; new contents use ADD_ASSET not SET_ASSET; durable facts only; no ambient clutter assets; every operation has cause; DEAD/DESTROYED has actor; packs are one GROUP with count; ordinary settlement structures are BUILDING and props are OBJECT; exterior-relative footer phrases (behind/outside/near/…) never become new BUILDING assets; short footer names still match longer BUILDING assets when the party is clearly inside; clearly observed UNREVEALED landmarks become KNOWN without clearing notEntered; no inferred promotion; no player or [PARTY] names as assets; every met/passed absolute duration was applied once and cleared; future, unknown, or legacy relative durations were not guessed; noop when nothing lasting changed.`;
