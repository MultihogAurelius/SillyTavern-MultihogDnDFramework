...
KIND: SETTLEMENT
A chapel, inn, shop, house, or similar ordinary structure the party actually enters is a BUILDING asset in the current district, not a new area or peer map. OBJECT is props only. BUILDING is a lightweight container with no rooms or connections. Contained occupants and things use the BUILDING id as location. Named people are CREATURE; unnamed bands, watches, and crowds are GROUP with count. Entering it is durable even when the district was already VISITED. If CURRENT LOCATION names an untracked ordinary structure (e.g. "Chapel of the Drowned Stone", "Hollow Creek General Store"), ADD_ASSET kind BUILDING, knowledge KNOWN. Positional footer tails such as "behind the general store", "outside the inn", "near the chapel", or "in front of the gas station" place the party in the district relative to a landmark — they are NOT a new BUILDING and do not mean the party entered that structure. Prefer the existing landmark asset when narration establishes a true first entry inside it; exterior-relative phrasing alone must never invent an asset named after the prepositional phrase. Streetscape observation is also durable: when RECENT STORY clearly identifies existing UNREVEALED or SUSPECTED BUILDING/OBJECT landmarks from outside (a general store, gas station, diner, chapel facade, and so on), SET_ASSET knowledge KNOWN on each matching asset id. Match by the asset's name even when the prose uses a shorter common label ("gas station" → Bullion Gas Station). Observation is not entry — do not clear notEntered or invent interiors unless the party actually goes inside. When FIRST-ENTRY BUILDING POPULATION is supplied, populate or intentionally resolve that exact building and explicitly SET_ASSET notEntered:false in the same transaction; never noop that bundle. SUBDUNGEON/SUBINTERIOR are allowed only when the narration explicitly establishes a map-worthy peer; never infer or perform BUILDING promotion. CreateAreaMap is the sole promotion signal.
